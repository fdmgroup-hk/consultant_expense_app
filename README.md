...
You need one thing to get started: an Anthropic API key from <https://console.anthropic.com/settings/keys>. Put it in `.env` as `ANTHROPIC_API_KEY`. Everything else has a working default.
...
|---|---|---|
| `ANTHROPIC_API_KEY` | — | Required for chat and interview practice |
...
Only the Claude API calls cost anything; storage and embeddings can be entirely free.

At `claude-opus-5` list pricing ($5 per million input tokens, $25 per million output):

- A chat answer with 8 retrieved excerpts is roughly 6k input + 800 output tokens — **about 5–6 US cents**.
- An interview question or a scored answer is similar.
- A 20-question practice session lands around **$1–2**.

Ways to cut it, in order of impact: set `ANTHROPIC_EFFORT=medium`, switch `ANTHROPIC_MODEL` to `claude-sonnet-5`, or lower `RETRIEVAL_TOP_K`. The stable system prompt is sent as a cacheable block, so repeated turns in one conversation get cheaper once the prefix passes the caching threshold.
